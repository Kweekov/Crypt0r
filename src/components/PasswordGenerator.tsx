    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">Длина
          <input type="number" value={length} min={6} max={128} onChange={e=>setLength(Number(e.currentTarget.value))} className="mt-1 input" />
        </label>
        <label className="block">Заглавные буквы
          <input type="checkbox" checked={useUpper} onChange={e=>setUseUpper(e.currentTarget.checked)} className="ml-2" />
        </label>
        <label className="block">Цифры
          <input type="checkbox" checked={useDigits} onChange={e=>setUseDigits(e.currentTarget.checked)} className="ml-2" />
        </label>
        <label className="block">Символы
          <input type="checkbox" checked={useSymbols} onChange={e=>setUseSymbols(e.currentTarget.checked)} className="ml-2" />
        </label>
      </div>
